...
SELECT name,indep_year,population
FROM countries
...
SELECT name,indep_year,code
FROM countries
WHERE indep_year <1990 AND code IN ('DZA');
...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(co.name, 'の人口は', co.population, '人です')
FROM countries co;
...
SELECT name,SUBSTRING(code,1,1)
FROM countries;
...
SELECT region,AVG(life_exPectancy) AS ave_life,AVG(population) AS ave_population
FROM countries
GROUP BY region;

-- 問22
-- 全ての地方の最長寿命、最大人口を表示してください。(NULLも表示)
SELECT region,MAX(life_exPectancy) AS max_life,MAX(population) AS max_population
FROM countries
GROUP BY region;

-- 問23
-- アジア大陸の中で最小の表面積を表示してください
SELECT MIN(surface_area) AS min_asia
FROM countries
WHERE continent = 'Asia';

-- 問24
-- アジア大陸の表面積の合計を表示してください。
SELECT SUM(surface_area) AS total_asia
FROM countries
WHERE continent = 'Asia';
...
SELECT cities.name,countries.name,country_languages.language
FROM cities
JOIN countries
...
SELECT ce.country_code AS celeb_code,ce.name AS celeb_name,co.code AS country_code,co.name AS country_name
FROM celebrities ce
LEFT JOIN countries co ON co.code = ce.country_code;
...
SELECT ce.name,
(SELECT co.name FROM countries co WHERE co.code = ce.country_code) AS country
FROM celebrities ce;
...
FROM countries co
JOIN celebrities ce ON ce.country_code = co.code
GROUP BY co.name
...
SELECT co.name AS country, AVG(ce.age) AS ave_age
FROM countries co
JOIN celebrities ce ON co.code = ce.country_code
GROUP BY co.name
ORDER BY ave_age DESC;
